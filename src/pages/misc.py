...
def get_misc_html_file(bbg_version, lang, pages_list):
    ...
    title = f'Civ VI {f"BBG {bbg_version}" if bbg_version != None else get_loc(locs_data, "LOC_BASE_GAME_TITLE")} {get_loc(locs_data, "LOC_PAGE_TITLE_MISC")}'
    ...
    for alliance in alliance_list:
        menu_items.append(get_loc(locs_data, alliance[1]))
        menu_icons.append(get_loc(en_US_locs_data, alliance[1]))
    ...
    def create_misc_page():
        for era in dedication_list_per_era.keys():
            with div(cls="row", id=get_loc(locs_data, era)), div(cls="col-lg-12"), div(cls="chart"):
                comment(era)
            ...
            with div(cls="row"), div(cls="chart"), div(cls="row"):
                for dedication in dedication_list_per_era[era]:
                    with div(cls="col-lg-3"):
                        ...
                        p(get_loc(locs_data, dedication[2]), style="text-align:left", cls='civ-ability-desc')
                        br()
                br()
            with div(cls="row"):
                for policy in dark_age_policy_per_era[eras_reverse_map[era]]:
                    with div(cls="col-lg-3"), div(cls="chart"):
                        ...
                        br()
        for alliance in alliance_list:
            with div(cls="row", id=get_loc(locs_data, alliance[1])), div(cls="col-lg-12"):
                comment(alliance[1])
                h2(get_loc(locs_data, alliance[1]), cls='civ-name')
                br()
                        ...
                        with div(cls="row"):
                            ...
                            for effect in alliance_effect[lvl]:
                                with div(cls=div_cls):
                                    with div(cls="chart"):
                                        comment(effect)
                                        p(get_loc(locs_data, effect), style="text-align:left", cls='civ-ability-desc')
                br()
