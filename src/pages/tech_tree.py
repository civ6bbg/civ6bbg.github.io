...
def get_tech_tree_html_file(bbg_version, lang, pages_list):
    ...
    title = f'Civ VI {f"BBG {bbg_version}" if bbg_version != None else get_loc(locs_data, "LOC_BASE_GAME_TITLE")} {get_loc(locs_data, "LOC_PAGE_TITLE_TECHNOLOGY_TREE")}'
    ...
    def create_tech_tree_page():
        for era in eras_list:
            ...
            with div(cls="row"):
                for tech in tech_per_era[era]:
                    with div(cls="col-lg-4 col-sm-6 col-12"), div(cls="chart"):
                        comment(tech[1])
                        with h3(f'{get_loc(locs_data, tech[1])}',
                                style="text-align:left",
                                cls='civ-ability-name'):
                            img(src=f'/images/techs/{get_loc(en_US_locs_data, tech[1]).replace(' ', '_')}.webp',
