...
def get_religion_html_file(bbg_version, lang, pages_list):
    ...
    title = f'Civ VI {f"BBG {bbg_version}" if bbg_version != None else get_loc(locs_data, "LOC_BASE_GAME_TITLE")} {get_loc(locs_data, "LOC_PAGE_TITLE_RELIGION")}'
    ...
    def create_religion_page():
        for religion_cls in religion_cls_elements.keys():
            with div(cls="row", id=get_loc(locs_data, religion_cls)):
                with div(cls="col-lg-12"), div(cls="chart"):
                    with h1(get_loc(locs_data, religion_cls), cls='civ-name'):
                        img(src=f'/images/religion/{get_loc(en_US_locs_data, religion_cls)}.webp',
                ...
                for elem in religion_cls_elements[religion_cls]:
                    with div(cls="col-lg-6"), div(cls="chart"):
                        if religion_cls == 'LOC_BELIEF_CLASS_PANTHEON_NAME':
                            comment(elem[1])
                            with h2(get_loc(locs_data, elem[1]), cls='civ-name'):
                                img(src=f'/images/religion/{get_loc(en_US_locs_data, elem[1])}.webp',
                                    style="vertical-align: middle; height:3em",
                                    onerror=image_onerror)
                        else:
                            comment(elem[1])
                            h2(get_loc(locs_data, elem[1]), cls='civ-name')
