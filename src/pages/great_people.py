...
def get_great_people_html_file(bbg_version, lang, pages_list):
    ...
    title = f'Civ VI {f"BBG {bbg_version}" if bbg_version != None else get_loc(locs_data, "LOC_BASE_GAME_TITLE")} {get_loc(locs_data, "LOC_PAGE_TITLE_GREAT_PEOPLE")}'
    ...
    for gp_type in great_people_list:
        menu_items.append(get_loc(locs_data, gp_type))
        menu_icons.append(get_loc(en_US_locs_data, gp_type))
                ...
                with div(cls='row'):
                    ...
                    for gp in great_people[gp_type][era]:
                        with div(cls=div_cls), div(cls="chart"):
                            comment(gp[1])
                            p(get_loc(locs_data, gp[1]), cls='civ-ability-name')
                            br()
                            if gp[4] > 0:
